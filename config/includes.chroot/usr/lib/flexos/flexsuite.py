...
import json, platform, re, shutil, subprocess, time, zipfile
...
def run(cmd,timeout=12):
    ...
    except subprocess.TimeoutExpired:return 124,"Zaman aşımı"
...
def hardware():
    d={"cpu":"Unknown","ram_gib":0.0,"gpus":[],"network":[],"battery":False,"virt":"none",
    ...
    try:
        for x in Path("/proc/cpuinfo").read_text(errors="ignore").splitlines():
            if x.lower().startswith("model name"):d["cpu"]=x.split(":",1)[1].strip();break
    ...
    if command("lspci"):
        ...
        for x in out.splitlines():
            low=x.lower()
            if any(v in low for v in ("vga compatible controller","3d controller","display controller")):d["gpus"].append(x.split(": ",1)[-1])
            if "network controller" in low or "ethernet controller" in low:d["network"].append(x.split(": ",1)[-1])
...
def hardware_text():
    h=hardware();a=[f"CPU: {h['cpu']}",f"RAM: {h['ram_gib']} GiB",f"Kernel: {h['kernel']}",f"Mimari: {h['arch']}",
        f"Boot: {h['boot']}",f"Sanallaştırma: {h['virt']}",f"Batarya: {'Var' if h['battery'] else 'Yok'}"]
...
def recommendations():
    ...
    if "nvidia" in gpu:r.append(("OK","NVIDIA sürücüsü etkin.")) if command("nvidia-smi") else r.append(("ACTION","NVIDIA GPU algılandı; Flex Driver Manager proprietary sürücüyü kurabilir."))
    if h["ram_gib"] and h["ram_gib"]<=16:r.append(("ACTION",f"{h['ram_gib']} GiB RAM için zRAM faydalı olabilir."))
    if h["battery"]:r.append(("TIP","Dizüstü algılandı; Balanced veya Battery profili önerilir."))
    fs=run(["findmnt","-n","-o","FSTYPE","/"])[1] if command("findmnt") else ""
    r.append(("ACTION","Btrfs algılandı; Flex Snapshot kullanılabilir.")) if fs=="btrfs" else r.append(("INFO","Snapshot rollback için Btrfs önerilir."))
    return r
...
def driver_status():
    ...
    if any("nvidia" in x.lower() for x in h["gpus"]):
        if command("nvidia-smi"):
            rc,o=run(["nvidia-smi","--query-gpu=name,driver_version","--format=csv,noheader"]);a.append("NVIDIA: "+(o if rc==0 else "sürücü komutu hata verdi"))
        else:a.append("NVIDIA proprietary driver: kurulu değil")
    elif any("amd" in x.lower() or "ati" in x.lower() for x in h["gpus"]):a.append("AMD: kernel/Mesa açık kaynak yığını.")
    elif any("intel" in x.lower() for x in h["gpus"]):a.append("Intel: kernel/Mesa açık kaynak yığını.")
    return "\n".join(a)
...
def snapshot_supported():return run(["findmnt","-n","-o","FSTYPE","/"])[1]=="btrfs"
def snapshot_status():
    if not snapshot_supported():return "Kök dosya sistemi Btrfs değil."
    if not command("snapper"):return "Snapper kurulu değil."
    return "Btrfs + Snapper hazır." if Path("/etc/snapper/configs/root").exists() else "Btrfs hazır; Snapper root config henüz oluşturulmadı."
...
def logs_summary():
    ...
    if command("journalctl"):
        _,o=run(["journalctl","-b","-p","warning","--no-pager","-n","120"],10);out.append("\nBOOT WARNINGS\n"+(o or "Yok / erişilemedi"))
    return "\n".join(out)
...
def enable_flathub_user():return run(["flatpak","remote-add","--user","--if-not-exists","flathub","https://flathub.org/repo/flathub.flatpakrepo"],60)
...
def create_backup(dest):
    ...
    with zipfile.ZipFile(dest,"w",zipfile.ZIP_DEFLATED) as z:
        z.writestr("manifest.json",json.dumps({"format":1,"created":int(time.time()),"profile":current_profile(),"desktop":"KDE Plasma"},indent=2))
        for p in [home/".config/flexos",home/".config/kdeglobals",home/".config/kwinrc",home/".config/plasmarc",home/".config/konsole",home/".local/share/konsole"]:
...
def report_text():
    return "\n".join(["FlexOS System Report","====================",hardware_text(),"","RECOMMENDATIONS",
        *[f"[{s}] {m}" for s,m in recommendations()],"","SECURITY",security_status(),"","DRIVERS",driver_status(),
        "","KERNEL",kernel_status(),"","SNAPSHOT",snapshot_status(),"","LOG SUMMARY",logs_summary()])
